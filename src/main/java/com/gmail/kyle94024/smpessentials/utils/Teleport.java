...
import com.gmail.kyle94024.smpessentials.data.Pair;
...
public class Teleport implements CommandExecutor {
    ...
    private static Map<Player, Pair<Player, Integer>> currentRequests = new HashMap<Player, Pair<Player, Integer>>();
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("tpa")) {
            ...
            if (args.length == 1) {
                ...
                sendRequest(player, targetPlayer, task);
            ...
            if (currentRequests.containsKey(target)) {
                Player requester = currentRequests.get(target).getFirst();
                int task = currentRequests.get(target).getSecond();
    ...
    public void sendRequest(Player requester, Player target, int task) {
        currentRequests.put(target, new Pair<Player, Integer>(requester, task));
